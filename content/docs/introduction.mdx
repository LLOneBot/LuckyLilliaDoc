...
（全称 LuckyLilliaBot, 幸运莉莉娅）是一个基于 NTQQ 的 QQ 机器人框架，
LLBot 实现了两种方式将 QQ 的功能暴露出来：

1.通过 PMHQ 将 QQ 的功能暴露出来

2.直接完整实现了 NTQQ 协议，不再依赖 QQ 客户端

之后再通过 LLBot 封装成标准协议通过 WS, HTTP 等形式暴露出来，供开发者编写自定义的应用逻辑使用。
...
<Mermaid chart={`flowchart LR
    A[NTQQ<br/>LLBot纯协议实现]
    A <--> B[LLBot适配器<br/>转换协议]
    B <--> C[机器人框架<br/>功能实现]
`} />
