...
```json5
{
  ...
  "milky": {
    "enable": true,  // 是否启用 Milky 协议
    "reportSelfMessage": false,  // 是否上报自己发送的消息
    "http": {
      "port": 3010, // Milky 的 WS 和 HTTP 实际上是共用一个端口的
...
下一步：基于上面的配置你可以选择[基于协议开发](./develop)，或者[使用现成框架对接](./install_framework)
